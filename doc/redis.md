...
2. redis的使用场景，项目中都用了哪些特性，会根据项目的实现情况问数据同步问相关问题

3. 如何实现一个分布式锁，如果保证锁被一个任务占用后超出时间后，其他任务是正常的

4. redis怎么实现主从同步，说出具体过程，并且需要注意些什么问题

5. redis的性能高的原因，说出IO多路复用的原理以及其他的优点

   

6. redis的持久化原理，说出bgsave以及aof等原理以及具体使用时的优缺点

   

7. redis如何保证高可用？ 哪里会出现不高可用
...
### redis主从机制了解么？怎么实现的？,断点续传，无磁盘化复制，过期key处理
